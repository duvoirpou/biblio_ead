...
if (isset($_POST["search"]["value"])) {
    ...
    $query .= 'OR d_rapp_stage.nom LIKE "%' . htmlentities(trim($_POST["search"]["value"])) . '%" ';
    $query .= 'OR d_rapp_stage.prenom LIKE "%' . htmlentities(trim($_POST["search"]["value"])) . '%" ';
...
foreach ($result AS $row) {
    ...
    $sub_array[] = '<a class="lien" style="text-decoration: none;" href="?c=cnxEtuDocs&code='.$row['id_doc'].'">
												<button  class="btn btn-outline-success btn-block btn-sm" type="submit" style=" margin-bottom: 10px"><i class="fa fa-book"></i> ATTRIBUER</button>
                                                
                                                
                                                <a class="lien" style="text-decoration: none;" href="?c=modifDocs&code='.$row['id_doc'].'">
												<button  class="btn btn-primary btn-block btn-sm" type="submit" style=" margin-bottom: 10px"><i class="fa fa-edit"></i> MODIFIER</button>
											</a>
                     <button  class="btn btn-danger btn-block btn-sm suppr" id="'.$row['id_doc'].'" type="submit" style=""><i class="fa fa-remove"></i> SUPPRIMER</button>                     
                                            ';
    $data[] = $sub_array;
}
